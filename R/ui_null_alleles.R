...
# Bootstrap CI over loci and over individuals (sub-samples)
#
# References:
...
null_alleles_UI <- function(id) {
  ...
  custom_css <- tags$style(HTML("
    ...
    /* ── Bootstrap panel (violet tint) ──────────────────────────────── */
    .na-panel-boot { background:#faf5ff; border:1px solid #e9d5ff; border-radius:9px; margin-bottom:.9rem; overflow:hidden; }
    .na-panel-boot-head {
      background:#f3e8ff; border-bottom:1px solid #e9d5ff;
      padding:.6rem .95rem; display:flex; align-items:center; flex-wrap:wrap;
    }
    .na-panel-boot-title {
      font-size:12.5px; font-weight:600; color:#4c1d95;
      display:flex; align-items:center; gap:6px; flex-wrap:wrap;
    }
    ...
    .na-info-boot {
      background:#faf5ff; border:1px solid #d8b4fe; border-radius:7px;
      padding:.5rem .85rem; font-size:11.5px; color:#4c1d95;
      display:flex; align-items:flex-start; gap:7px;
      margin-bottom:.9rem; line-height:1.75;
    }
    .na-warn {
    ...
    .na-module table.dataTable thead th {
      ...
      font-size:11px !important; font-weight:600 !important; letter-spacing:.03em !important;
    }
  ...
  # Bootstrap panel — shared across tabs 3, 4, 5
  # type: "loci_only" | "both"
  boot_panel <- function(run_id, result_id, dl_csv_id, dl_txt_id, type = "both") {
    choices_boot <- if (type == "loci_only")
    ...
    tags$div(class = "na-panel-boot",
      ...
      tags$div(class = "na-panel-body",
        tags$div(class = "na-info-boot",
          icon("info-circle"),
          tags$div(
            tags$strong("Bootstrap over loci:"),
            " resample loci with replacement \u2014 95% CI on the multilocus statistic.",
            tags$br(),
            if (type != "loci_only") tagList(
            ...
            "5 000 replicates \u00b7 percentile method \u00b7 vectorised R (no loop) \u2014 runs in a few seconds."
          )
        ),
        fluidRow(
          column(3,
        ...
        conditionalPanel(
          condition = sprintf("output['%s'] != ''", ns(paste0(result_id, "_ready"))),
  ...
  tags$div(class = "na-module",
    ...
    # ── tabsetPanel — all 6 tabs + bootstrap ──────────────────────────────────
    tabsetPanel(
      ...
      tabPanel(
        ...
        tags$div(class = "na-panel",
          tags$div(class = "na-panel-head",
            tags$div(class = "na-panel-title", icon("sliders-h"), " Filter parameters")),
          tags$div(class = "na-panel-body",
            fluidRow(
              column(3, selectInput(ns("t1_locus"), "Locus:",
                choices = c("All loci" = "all"), selected = "all")),
              column(3, selectInput(ns("t1_pop"), "Population:",
        ...
        tags$div(class = "na-panel",
          tags$div(class = "na-panel-head",
            tags$div(class = "na-panel-title",
              icon("list"), " Estimating null allele frequency using the EM algorithm (Dempster et al. 1977)",
              tags$span(style = "font-size:10.5px; color:#64748b; margin-left:8px; font-weight:400;",
      ...
      tabPanel(
        ...
        tags$div(class = "na-info", icon("info-circle"),
          tags$div(
            ...
            tags$br(),
            tags$strong("Av(N_exp_blanks)"), " = \u03a3(N\u1d62 \u00d7 p\u1d62\u00b2): total expected null homozygotes across all populations.",
            tags$br(),
        ...
        tags$div(class = "na-panel",
          ...
          tags$div(class = "na-panel-body",
            fluidRow(
              column(3, selectInput(ns("t2_locus"), "Locus:",
                choices = c("All loci" = "all"), selected = "all")),
              column(3, tags$div(style = "margin-top:25px;",
        ...
        tags$div(class = "na-panel",
          tags$div(class = "na-panel-head",
            tags$div(class = "na-panel-title",
              icon("globe"), " Global null allele frequency per locus",
              tags$span(style = "font-size:10.5px; color:#64748b; margin-left:8px; font-weight:400;",
      ...
      # ════════════════════════════════════════════════════════════════════════ #
      # TAB 3  Global FST-ENA (multilocus) + Bootstrap                          #
      # ════════════════════════════════════════════════════════════════════════ #
      tabPanel(
        ...
        tags$div(class = "na-info-teal", icon("info-circle"),
          tags$div(
            ...
            tags$br(),
            tags$strong("Raw FST"), ": computed from observed allele frequencies (null homozygotes excluded from the denominator).",
            tags$br(),
        ...
        tags$div(class = "na-compare-grid",
          tags$div(class = "na-compare-card",
            ...
            tags$div(class = "na-compare-body",
              ...
              tags$br(), "May be upwardly biased when null alleles are present.")
          ),
          tags$div(class = "na-compare-card",
            ...
            tags$div(class = "na-compare-body",
              ...
              tags$br(), "Bias due to null homozygotes is removed.")
          )
        ),

        tags$div(class = "na-panel",
          tags$div(class = "na-panel-head",
            tags$div(class = "na-panel-title", icon("sliders-h"), " Parameters")),
          tags$div(class = "na-panel-body",
            fluidRow(
              column(3, tags$div(style = "margin-top:5px;",
        ...
        tags$div(class = "na-panel",
          tags$div(class = "na-panel-head",
            tags$div(class = "na-panel-title",
              icon("list"), " Global multilocus FST \u2014 per locus (raw and ENA-corrected)",
              tags$span(style = "font-size:10.5px; color:#64748b; margin-left:8px; font-weight:400;",
        ...
        # ── Bootstrap panel — global FST (over loci only) ──────────────────
        boot_panel(
          run_id    = "run_boot_fst_global",
          result_id = "ui_boot_fst_global",
          dl_csv_id = "dl_boot_fst_global_csv",
          dl_txt_id = "dl_boot_fst_global_txt",
          type      = "loci_only"
        )
      ),
      ...
      tabPanel(
        ...
        tags$div(class = "na-panel",
          tags$div(class = "na-panel-head",
            tags$div(class = "na-panel-title", icon("th"), " Pairwise FST matrix \u2014 lower triangle")),
          tags$div(class = "na-panel-body",
        ...
        # ── Bootstrap panel — pairwise FST (loci + individuals) ────────────
        boot_panel(
          run_id    = "run_boot_fst_pair",
          result_id = "ui_boot_fst_pair",
          dl_csv_id = "dl_boot_fst_pair_csv",
          dl_txt_id = "dl_boot_fst_pair_txt",
          type      = "both"
        )
      ),

      # ════════════════════════════════════════════════════════════════════════ #
      # TAB 5  Pairwise DCSE-INA + Bootstrap                                    #
      # ════════════════════════════════════════════════════════════════════════ #
      tabPanel(
        ...
        tags$div(class = "na-info-teal", icon("info-circle"),
          tags$div(
            ...
            tags$br(),
            tags$strong("Raw DCSE"), ": computed from observed allele frequencies (null allele state excluded).",
            tags$br(),
        ...
        tags$div(class = "na-panel",
          tags$div(class = "na-panel-head",
            tags$div(class = "na-panel-title", icon("th"), " Pairwise DCSE matrix \u2014 lower triangle")),
          tags$div(class = "na-panel-body",
        ...
        # ── Bootstrap panel — pairwise DCSE (loci + individuals) ───────────
        boot_panel(
          run_id    = "run_boot_dc",
          result_id = "ui_boot_dc",
          dl_csv_id = "dl_boot_dc_csv",
          dl_txt_id = "dl_boot_dc_txt",
          type      = "both"
        )
      ),
      ...
      tabPanel(
        ...
        tags$div(class = "na-panel",
          tags$div(class = "na-panel-head",
            tags$div(class = "na-panel-title", icon("sliders-h"), " Filters")),
          tags$div(class = "na-panel-body",
            fluidRow(
